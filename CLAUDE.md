...
- `main.dart`'s `AuthGate` is the only role-based routing: not logged in -> `WelcomeScreen` (branded entry point offering login or register); logged in as vendor -> `VendorDashboardScreen`; logged in as customer -> `MarketMapScreen` (the plain `VendorListScreen` is still reachable from its app bar, not deleted).
- `lib/features/customer` — `MarketMapScreen` (customer home): stalls positioned in a deterministic grid by list index, tap-to-move avatar (`AnimatedPositioned`), tapping a stall walks there and opens it, and walking near a stall without tapping it directly auto-opens its menu too (`_maybeAutoOpenNearbyVendor`, edge-triggered via `_lastNearVendorId` so it fires once per approach, not every frame within range) — a game-ish touch with no functional gate (every stall is always tappable regardless). Built from plain widgets (`CustomPainter`, `Icon`/`Container`) rather than an illustrated map or a game engine like Flame, since this sandbox can't fetch image assets or verify an untested new package. From there: stall menu (add to cart) -> cart/checkout (`CreateOrder` always returns `awaiting_payment`) -> order status, which shows a PromptPay QR to scan while `awaiting_payment` and switches to the pickup code once payment clears (polls every 5s, which doubles as the payment-status poll — see backend notes above) -> order history.
- `lib/features/customer/market_game` — experimental re-implementation of the market map on [Flame](https://flame-engine.org), reachable from `MarketMapScreen`'s app bar rather than replacing it. `flame` is the first external package this app has ever added; unlike the rest of `app/`, it hasn't even had a chance to be exercised by `flutter pub get` here (no network to `pub.dev`), so treat it as more unverified than everything else in this already-unverified app. The camera follows the player vertically and clamps to the map's extent by hand in `update()` (`camera.follow` + manual `camera.viewfinder.position.y` clamping in `market_flame_game.dart`, since `camera.setBounds`'s bounds-shape API didn't match the resolved Flame version) so maps taller than the viewport scroll instead of clipping, and each `StallComponent` tracks its own `wasNear` flag so walking close auto-opens its menu the same way the widget version does. See app/README.md's "Experimental: Flame version" for details and why `market_map_game_screen_test.dart` passes `settle: false` to `pumpGolden`.
