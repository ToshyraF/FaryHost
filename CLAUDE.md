...
- `lib/features/customer/market_game` — experimental re-implementation of the market map on [Flame](https://flame-engine.org), reachable from `MarketMapScreen`'s app bar rather than replacing it. `flame` is the first external package this app has ever added; unlike the rest of `app/`, it hasn't even had a chance to be exercised by `flutter pub get` here (no network to `pub.dev`), so treat it as more unverified than everything else in this already-unverified app. The camera follows the player vertically (`camera.follow` + `camera.setBounds` in `market_flame_game.dart`) so maps taller than the viewport scroll instead of clipping. See app/README.md's "Experimental: Flame version" for details and why `market_map_game_screen_test.dart` passes `settle: false` to `pumpGolden`.
