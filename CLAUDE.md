...
- `main.dart`'s `AuthGate` is the only role-based routing: not logged in -> `WelcomeScreen` (branded entry point offering login or register); logged in as vendor -> `VendorDashboardScreen`; logged in as customer -> `MarketMapGameScreen` (the plain `VendorListScreen` is still reachable from its app bar, not deleted).
