...
- `main.dart`'s `AuthGate` is the only role-based routing: not logged in -> `LoginScreen`; logged in as vendor -> `VendorDashboardScreen`; logged in as customer -> `VendorListScreen`.
