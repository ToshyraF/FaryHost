...
- **Persistence is in-memory** (`backend/internal/store`), not Postgres —
  ...
  restart.
- **The Flutter app has not been run or analyzed.** No Dart/Flutter SDK was
  available to verify it. `app/lib` was written and manually cross-checked
  against the backend's request/response shapes, but treat it as unverified
  until `flutter analyze` / `flutter run` actually pass in a real
  environment. `app/android`, `app/ios`, `app/web` etc. don't exist yet —
  they're generated by `flutter create .`, not hand-written.
